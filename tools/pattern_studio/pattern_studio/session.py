...
from .engine import apply_strand_config, make_strand
...
class StrandSession(QObject):
    ...
    def __init__(self, config: StrandConfig, parent=None):
        super().__init__(parent)
        self.config = config
        self.strand = make_strand(config)
    ...
    def rebuild(self) -> None:
        ...
        self.strand = make_strand(self.config)
        if was_running:
            self.timer.start()
        self.ticked.emit()
